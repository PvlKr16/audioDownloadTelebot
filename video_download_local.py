...
def print_formats(video_formats, formats_blacklist):
    ...
    for i, f in enumerate(video_formats, start=1):
        ...
        print(f"{len(available)+1:>2}. {fmt_id:>4}: {f.get('ext','?')} — {f.get('format_note','')} — {f.get('resolution','N/A')} "
              f"— {audio_info} — {size_str}")
        available.append(f)
    return available
...
def main():
    ...
    while True:
        ...
        if not choice:
            ...
            # Если видео без звука — предлагаем варианты, как прежде
        ...
        try:
            ...
            print(err_msg)
            # Если пользователь выбрал конкретный формат — заносим его в blacklist
            if chosen_video_format_id:
                ...
                # авто-режим упал — больше не пробуем авто, предлагаем выбрать вручную
                print("Рэжым аўта (bestvideo+bestaudio) не спрацаваў. Калі ласка, выберыце фармат уручную.")
                auto_mode_blocked = True

            # Дополнительно: если в тексте ошибки содержится код 403 — подсказка пользователю
